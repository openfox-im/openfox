...
**Intent → Plan → Approval → Execution → Receipt**
...
```bash
openfox intent transfer --to 0x... --amount 5
openfox intent status <intent-id>
openfox intent list
openfox intent explain <intent-id>
openfox intent replay <intent-id>
...
```bash
openfox terminal list
openfox terminal sessions
openfox terminal revoke <session-id>
openfox terminal policy <terminal-class>
```
...
- Policy templates for individual, merchant, institutional, and custodial accounts
- Policy simulation with 15 standard scenarios (overspend, terminal mismatch, escalation trigger, etc.)
...
```bash
openfox policy list
openfox policy show <policy-id>
openfox policy simulate --policy <id> --scenario overspend
openfox policy create --template merchant-medium
openfox policy explain <policy-id>
openfox policy diff <policy-a> <policy-b>
...
- Append-only audit journal for every financial action
- Replay inspector for reproducing execution paths
- Proof display for receipts and settlement records
- Report generation for compliance and operator review
- Dispute resolution tooling

CLI surface:

```bash
openfox audit journal --from 2025-01-01 --to 2025-03-01
openfox audit report --period weekly
...
- audit journal with replay, proof display, and dispute resolution
...
- [OpenFox Ecosystem Architecture](./docs/OpenFox-Ecosystem-Architecture.md)
...
- intent pipeline with Intent → Plan → Approval → Receipt lifecycle
- six terminal adapters (mobile app, NFC card, POS, voice, kiosk, robot API)
- policy authoring with 8 templates, simulation, and human-readable explanations
- append-only audit journal with replay inspection and proof display
...
- dispute resolution tooling for the audit journal
- expanded policy scenario coverage
