...
export interface BundledTemplateInfo {
  ...
  exportMode: "copy" | "generated";
  generatedBy?: string;
}

interface BundledTemplateMetadata {
  exportMode?: "copy" | "generated";
  generatedBy?: string;
}

const TEMPLATE_METADATA: Record<string, BundledTemplateMetadata> = {
  "metaworld-local-demo": {
    exportMode: "generated",
    generatedBy: "openfox world demo export",
  },
};
...
export function listBundledTemplates(): BundledTemplateInfo[] {
  ...
  return fs
    ...
    .map((entry) => {
      ...
      const metadata = TEMPLATE_METADATA[entry.name] ?? {};
      return {
        name: entry.name,
        path: templatePath,
        hasReadme: fs.existsSync(readmePath),
        description: extractDescription(readmePath),
        exportMode: metadata.exportMode ?? "copy",
...
export async function exportBundledTemplate(params: {
  ...
  const metadata = TEMPLATE_METADATA[params.name] ?? {};
  if (!fs.existsSync(sourcePath) || !fs.statSync(sourcePath).isDirectory()) {
    throw new Error(`Unknown bundled template: ${params.name}`);
  }
  const outputPath = path.resolve(params.outputPath);
  if (metadata.exportMode === "generated") {
...
`);
  ...
  if (command === "list") {
    ...
    for (const item of items) {
      ...
      if (item.generatedBy) {
        logger.info(`  generated by: ${item.generatedBy}`);
      }
    }
    return;
  }
  ...
  if (command === "export") {
    ...
    logger.info(
      [
        ...
        result.generatedBy ? `Generated by: ${result.generatedBy}` : null,
      ]
