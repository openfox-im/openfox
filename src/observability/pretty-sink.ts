...
const PREFIX_STYLES: Array<[RegExp, (label: string, rest: string) => string]> = [
  ...
  [/^\[TOOLS?\]/, (l, r) => chalk.magenta(l) + " " + chalk.white(r)],
  [/^\[TOOL RESULT\]/, (l, r) => {
  ...
  [/^\[HEARTBEAT\]/, (l, r) => chalk.green(l) + " " + chalk.dim(r)],
...
function formatMessage(message: string): string {
  const match = message.match(/^(\[[A-Z][A-Z _-]*\])(.*)/s);
  if (match) {
    const label = match[1];
...
export function prettySink(entry: LogEntry): void {
  try {
    ...
    const mod = chalk.dim(entry.module.padEnd(16));
